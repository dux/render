...
@Render = 
  ...
  to: (node, opts, data) ->
    ...
    # opts.template ||= opts.__template if opts.__template
    # opts.__template = opts.template if typeof(opts.template) == 'function'
    template = opts.template

    # console.log opts.__template
  ...
  render: (opts)->
    ...
    if opts.__template
      opts.template = opts.__template

    first_node.data('scope', opts)

    opts.refresh = =>
      Render.refresh(first_node)
    ...
    opts.body.exportByAttr 'bind', (node, bind_var) =>
      ...
      if bind_var in ['data', 'template', 'bind', 'once', 'onchange','node', 'script']
  ...
  onchange_default: (opts) ->
    for key,node of opts.node
      if node[0].nodeName in ['TEXTAREA', 'INPUT', 'SELECT']
